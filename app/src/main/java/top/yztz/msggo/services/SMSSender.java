...
public class SMSSender {
    ...
    public static void sendMessage(Context context, String content, String phoneNumber, int subId, int code) {
        // فحص القيم الفارغة لمنع التعطل
        if (content == null || content.isEmpty() || phoneNumber == null || phoneNumber.isEmpty()) {
            android.util.Log.e("SMSSender", "Content or phone number is empty, skipping send");
            sendFailureBroadcast(context, code, phoneNumber);
            return;
        }
        ...
        try {
            ...
            // إرسال إشعار فشل فوري بدلاً من الانتظار 30 ثانية
            sendFailureBroadcast(context, code, phoneNumber);
        }
    }

    /**
     * إرسال إشعار فشل فوري لإخبار التطبيق أن الرسالة فشلت
     */
    private static void sendFailureBroadcast(Context context, int code, String phoneNumber) {
        Intent failIntent = new Intent(SENT_SMS_ACTION);
        failIntent.setPackage(context.getPackageName());
        failIntent.putExtra("code", code);
        failIntent.putExtra("phone", phoneNumber != null ? phoneNumber : "");
        failIntent.putExtra("part", 0);
        failIntent.putExtra("totalParts", 1);
